    ...
    for i in range(config["num_iterations"]):
        if i % config["test_interval"] == config["test_interval"] - 1 or i == 1:
            ...
            with torch.no_grad():
                ...
                log_str = "iter: {:05d}, precision: {:.5f}, a_distance: {:.5f}, rsl: {:.5f}, classifier_loss: {:.5f},  discriminator_loss: {:.5f},".format(i, temp_acc, a_distance, rsl.item(),classifier_loss.item(),transfer_loss.item())
        ...
        total_loss = loss_params["trade_off"] * transfer_loss + classifier_loss + rsl_loss
        total_loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Adversarial Spectral Adaptation Network')
    ...
    parser.add_argument('--s_dset_path', type=str, default='data/Office-31/amazon.txt', help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='data/Office-31/webcam.txt', help="The target dataset path list")
